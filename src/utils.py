...
def compute_mention_f1(y_true, y_pred):
    ...
    @param y_true (list): List of 7 tensors each of shape (dev_set_size)
    @param y_pred (list): Same as y_true but for model predictions

    @returns res (list): List of 7 scalars
    """
...
def compute_negation_f1(y_true, y_pred):
    ...
    for j in range(len(y_true)):
        ...
        # y_true[j][y_true[j] == 2] = 1
        # y_pred[j][y_pred[j] == 2] = 1
        y_true[j][y_true[j] == 1] = 1
        y_pred[j][y_pred[j] == 1] = 1
...
def evaluate(model, dev_loader, device, f1_weights, return_pred=False, charbert=False):
    ...
    @returns res_dict (dictionary): dictionary with keys 'blank', 'mention', 'negation',
                           'uncertain', 'positive' and 'weighted', with values 
                            being lists of length 7 with each element in the 
    ...
    weighted = []
    kappas = []
    # print(len(y_true), len(y_pred), len(negation_f1), len(uncertain_f1), len(positive_f1))
    for j in range(len(y_pred)):
        cond = CONDITIONS[j]
        # avg = weighted_avg([negation_f1[j], uncertain_f1[j], positive_f1[j]], f1_weights[cond])
        avg = weighted_avg([negation_f1[j], positive_f1[j]], f1_weights[cond])
        weighted.append(avg)
...
def test(model, checkpoint_path, test_ld, f1_weights, charbert=False):
    ...
    metrics = evaluate(model, test_ld, device, f1_weights, charbert)
    ...
    neg_macro_avg = np.mean(metrics['negation']) #No Finding has no negations
    pos_macro_avg = np.mean(metrics['positive'])
    ...
    #                                                                                                                 unc_macro_avg,
    #                                                                                                                 pos_macro_avg))
    print("negation macro avg: %.3f, positive macro avg: %.3f" % (neg_macro_avg,
                                                                  pos_macro_avg))
    # print()
    # for j in range(len(CONDITIONS)):
...
def label_report_list(checkpoint_path, report_list, charbert=False):
