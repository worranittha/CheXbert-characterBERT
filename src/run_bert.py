...
from src.dataset.impressions_dataset import ImpressionsDataset
...
def train(save_path, dataloaders, f1_weights, model=None, device=None,
          ...
          valid_niter=VALID_NITER, best_metric=0.0, charbert=False):
     ...
     for epoch in range(NUM_EPOCHS):
          for i, data in enumerate(train_ld, 0):
               ...
               label = data['label']                                                                     # tensor size: (batch_size, 7)
               ...
               if (i+1) % log_every == 0:
                    ...
                    log = {'epoch': epoch+1, 
                           'iter': i+1, 
                           'avg_loss': (report_loss/report_examples).item(), 
                           'time_elapsed': time.time() - begin_time}
                    if epoch+1 == 1 and i+1 == 200:
                         train_eval = [log]
                    else:
                         train_eval = json.load(open('./log/log_train.json'))
                         train_eval.append(log)                         
                    with open('./log/log_train.json', 'w') as f:
               ...
               if (i+1) % valid_niter == 0:
                    print('\n begin validation')
                    # print(charbert)
                    metrics = utils.evaluate(model, dev_ld, device, f1_weights, charbert=charbert)
                    ...
                    if epoch+1 == 1:
                         ...
                         valid_eval = json.load(open('./log/log_valid.json'))
                         valid_eval.append(metrics)
                    with open('./log/log_valid.json', 'w') as f:
                         json.dump(valid_eval, f)
                    ...
                    for j in range(len(CONDITIONS)):
                         print('%s negation_f1: %.3f, positive_f1: %.3f' % (CONDITIONS[j],
                                                                            metrics['negation'][j],
                                                                            metrics['positive'][j]))
                    ...
                    if metric_avg > best_metric:                                                         # new best network
                         ...
                         torch.save({'epoch': epoch+1,
                                    ...
                                    path)
          metric_epoch = best_metric
          if metric_epoch > best_metric_epoch:
...
def model_from_ckpt(model, ckpt_path, charmodel=None, cxrmodel=None, edited_model=False):
     ...
     @param charmodel (model): CharacterBERT model
     @param cxrmodel (model): CXR-BERT model
     @param edited_model (bool): whether model is already modified or not
     ...
     if torch.cuda.device_count() > 1:
          ...
          model = nn.DataParallel(model) #to utilize multiple GPU's
          model = model.to(device)
          checkpoint = torch.load(ckpt_path, map_location=device)                                        # load checkpoint from file
          ...
          if not edited_model:
               ...
               model.load_state_dict(new_state_dict, strict=False)                                       # add strict=False (because pretrained model does not have bert.embedding.position_ids)
                                                                                                         # -> Missing key(s) in state_dict: "bert.embeddings.position_ids"
     optimizer = torch.optim.Adam(model.parameters())
     optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

     # edit last layer and add .cuda() to use parameters from GPU
     hidden_size = model.bert.pooler.dense.in_features
     model.linear_heads = nn.ModuleList([nn.Linear(hidden_size, 2, bias=True) for _ in range(7)]).cuda()
     
     # use characterBERT
     if charmodel != None:
          print('use CharacterBERT')
          # model.bert.embeddings.word_embeddings = charmodel.embeddings.word_embeddings
          model.bert.embeddings = charmodel.embeddings
          # print(charmodel)     
     
     # use CXR-BERT-specialized
     # if cxrmodel != None:
     #      print('use CXR-BERT-specialized')
     #      model.bert.embeddings = cxrmodel.bert.embeddings
          # print(charmodel)
     ...
     # freeze bert (fine-tune only specific layer)
     # embed_layer = [model.bert.embeddings.position_embeddings, model.bert.embeddings.token_type_embeddings]
     # for module in embed_layer:
...
if __name__ == '__main__':
     parser = argparse.ArgumentParser(description='Train CheXbert/CheXcharacterBERT model for labeling 7 medical conditions.')
     ...
     # parser.add_argument('--cxrbert', type=str, nargs='?', required=False,
     #                     help='condition for using CX-BERT-specialized')
     # parser.add_argument('--edited_model', type=bool, nargs='?', required=False,
     #                     help='condition for using already edited model')
     ...
     # cxrbert = args.cxrbert
     # edited_model = args.edited_model

     # load CharacterBERT model (medical corpus)
     charmodel = None
     if charbert:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
     ...
     # if cxrbert:
     #    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
     #    cxrmodel = AutoModel.from_pretrained('microsoft/BiomedVLP-CXR-BERT-specialized', trust_remote_code=True)
     #    cxrmodel = cxrmodel.to(device)

     if checkpoint_path:
        model, optimizer, device = model_from_ckpt(bert_labeler(), checkpoint_path, charmodel=charmodel)
     else:
